...
# Specify the path to the model (local weights)
...
def generate_dola_static(
    ...
    """DoLa-static variant: fixed contrast layer j* with APC, no early-exit."""
    input_ids = vl_chat_processor.tokenizer.encode(prompt)
    input_ids = torch.LongTensor(input_ids).to(mmgpt.device)
    ...
    short_prompt = re.sub(r'\W+', '_', short_prompt)[:50]
    for i in range(parallel_size):
        save_path = os.path.join('generated_samples', f"img_dola_static_{timestamp}_{short_prompt}_{i}.jpg")
        PIL.Image.fromarray(visual_img[i]).save(save_path)
...
def interactive_image_generator():
    ...
    while True:
        ...
        print(f"Generating {parallel_size} image(s) for: '{user_input}' (DoLa-static)")
        generate_dola_static(
            mmgpt=vl_gpt,
            vl_chat_processor=vl_chat_processor,
            prompt=prompt,
            short_prompt=short_prompt,
            parallel_size=parallel_size,  # user-specified batch size
            # defaults: temperature=1, cfg_weight=5, j_star=24, alpha=0.1
        )
